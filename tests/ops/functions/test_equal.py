...
class TestMeasurementsEqual:
    ...
    def test_mid_measure(self):
        ...
        mp = qp.ops.MidMeasure(wires=Wires([0]), reset=True, meas_uid="test_id")

        mp1 = qp.ops.MidMeasure(wires=Wires([1]), reset=True, meas_uid="test_id")
        mp2 = qp.ops.MidMeasure(wires=Wires([0]), reset=False, meas_uid="test_id")
        mp3 = qp.ops.MidMeasure(wires=Wires([0]), reset=True, meas_uid="foo")
        ...
        assert (
            qp.equal(
                mp,
                qp.ops.MidMeasure(wires=Wires([0]), reset=True, meas_uid="test_id"),
            )
            is True
        )
    ...
    def test_pauli_measure(self):
        ...
        assert qp.equal(mp, PauliMeasure("XY", wires=Wires([0, 1]), meas_uid="test_id")) is True
...
def test_ops_with_abstract_parameters_not_equal():
    ...
    with pytest.raises(AssertionError, match="Data contains a tracer"):
        jax.jit(assert_equal)(qp.exp(qp.X(0), 0.5), qp.exp(qp.X(0), 0.5))

    assert not jax.jit(qp.equal)(qp.X(0) * 0.5, qp.X(0) * 0.5)
    with pytest.raises(AssertionError, match="Data contains a tracer"):
...
class TestCompareSubroutines:
    def test_different_subroutine_defs(self):
        ...
        op1 = qp.tape.make_qscript(Subroutine1)(0)[0]
        op2 = qp.tape.make_qscript(Subroutine2)(0)[0]
    ...
    def test_different_static_args(self):
        ...
        op2 = qp.tape.make_qscript(f)("val2", 0)[0]
    ...
    def test_different_wires(self):
        ...
        op2 = qp.tape.make_qscript(f)((1,), (0,))[0]
        assert not qp.equal(op1, op2)
    ...
    def test_different_data(self):
        ...
        op2 = qp.tape.make_qscript(f)(np.array(0.5 + 1e-5), 0)[0]

        assert not qp.equal(op1, op2)
        assert qp.equal(op1, op2, rtol=1e-4)
        assert qp.equal(op1, op2, atol=1e-4)

        op3 = qp.tape.make_qscript(f)(qp.numpy.array(0.5, requires_grad=False), 0)[0]
        assert not qp.equal(op1, op3)
        assert qp.equal(op1, op3, check_interface=False)

        op4 = qp.tape.make_qscript(f)(qp.numpy.array(0.5), 0)[0]
        assert not qp.equal(op3, op4)
        assert qp.equal(op3, op4, check_trainability=False)
    ...
    def test_labelled_op_comparison(self, base):
        ...
        op3 = LabelledOp(qp.PauliX(15), "my-base")
